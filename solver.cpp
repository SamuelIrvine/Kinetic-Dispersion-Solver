...
static array<array<T, N1+N2-1>, M1+M2-1> polyMul(const array<array<T, N1>, M1> &a1, const array<array<T, N2>, M2> &a2){
...
public://TODO: private later
    ...
    Species(const boost::python::object &species, double B){
        ...
        for (int i=0; i < len(pyns); ++i){
            ...
            q_ms.push_back({arr1d(npara-1), arr1d(npara-1), arr1d(npara-1), arr1d(npara-1), arr1d(npara-1), arr1d(npara-1)});
            q_cs.push_back({arr1d(npara-1), arr1d(npara-1), arr1d(npara-1), arr1d(npara-1), arr1d(npara-1), arr1d(npara-1)});
        ...
        for (size_t ni=0;ni<ns.size();ni++){
            ...
            for (size_t i=0;i<npara;i++){
                ...
                for (size_t j=0;j<nperp-1;j++){
                    ...
                    qh[5]+=jnps[ni][j]*jnps[ni][j]*vperp_h[j]*vperp_h[j]*vperp_h[j]*df_dvpara_h[i][j]*dvperp_h[j];
                }
